"""
ربات محافظ گروه فوق‌پیشرفته Luffy Shield - نسخه 7.0.0
...
class Database:
    ...
    def _create_tables(self):
        ...
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS settings (
                ...
                auto_delete_links INTEGER DEFAULT 1,
    ...
    def toggle_setting(self, group_id, key):
        current = self.get_setting(group_id, key)
        new_value = 0 if current else 1
        self.update_setting(group_id, **{key: new_value})
        return new_value
...
class ShieldBot:
    ...
    def _register_handlers(self):
        ...
        def logs_cmd(message):
            self.logs_command(message)
        
        @bot.message_handler(commands=['rules'])
        def rules_cmd(message):
            self.rules_command(message)
...
""")
    ...
    def help_command(self, message):
        bot.reply_to(message, """
🛡️ <b>راهنمای کامل Luffy Shield</b>
━━━━━━━━━━━━━━━━━━━━━━

<b>📌 مدیریت کاربران:</b>
/ban [@user/id] [دلیل] - بن کردن
/unban [@user/id] - رفع بن
/mute [@user/id] [دقیقه] [دلیل] - سکوت
/unmute [@user/id] - رفع سکوت
/warn [@user/id] [دلیل] - اخطار
/unwarn [@user/id] - حذف اخطار
/kick [@user/id] - اخراج

<b>📌 مدیریت پیام:</b>
/purge [تعداد] - پاکسازی پیام‌ها
/rules - نمایش قوانین
/welcome [پیام] - تنظیم پیام خوش‌آمدگویی

<b>📌 فیلترها:</b>
/blockword [کلمه] - افزودن کلمه ممنوع
/unblockword [کلمه] - حذف کلمه ممنوع
/allowedomain [دامنه] - افزودن دامنه مجاز
...
<b>💡 نکته:</b> برای شناسایی کاربر می‌توانید:
• از @username استفاده کنید
...
""")
    ...
    def setup_command(self, message):
        ...
        bot.reply_to(message, """
✅ <b>تنظیمات سریع اعمال شد!</b>
━━━━━━━━━━━━━━━━━━━━━━
🛡️ ضداسپم: فعال
🔗 ضدلینک: فعال
📝 کلمات ممنوع: 6 کلمه
⚠️ حداکثر اخطار: 3
🚨 اقدام بعد از اخطار: بن

برای تنظیمات بیشتر از /settings استفاده کنید.
━━━━━━━━━━━━━━━━━━━━━━
""")
    ...
    def settings_command(self, message):
        ...
        settings = {
            ...
            'زمان اسپم (ثانیه)': db.get_setting(chat_id, 'spam_timeout'),
            'حداکثر اخطار': db.get_setting(chat_id, 'max_warns'),
            'اقدام بعد اخطار': db.get_setting(chat_id, 'warn_action'),
            'خوش‌آمدگویی': db.get_setting(chat_id, 'welcome_enabled'),
            'خداحافظی': db.get_setting(chat_id, 'goodbye_enabled'),
            'کپچا': db.get_setting(chat_id, 'captcha_enabled'),
            'حذف خودکار اسپم': db.get_setting(chat_id, 'auto_delete_spam'),
    ...
    def _get_user_id(self, message, target):
        """پیدا کردن آیدی کاربر - نسخه کامل و تست شده"""
...
📝 دلیل: {reason}
👮 بن‌کننده: {message.from_user.first_name}
━━━━━━━━━━━━━━━━━━━━━━
""")
            
            try:
                bot.send_message(user_id, f"""
⛔ <b>شما از گروه بن شدید!</b>
━━━━━━━━━━━━━━━━━━━━━━
گروه: {message.chat.title}
دلیل: {reason}
...
""")
    ...
    def mute_command(self, message):
        ...
        if len(args) < 2:
            bot.reply_to(message, "⚠️ <b>فرمت:</b>\n<code>/mute [کاربر] [دقیقه] [دلیل]</code>")
            return
        ...
        try:
            ...
            db.execute("INSERT INTO muted_users (group_id, user_id, reason, muted_at, muted_until, muted_by) VALUES (?, ?, ?, ?, ?, ?)",
                      (chat_id, user_id, reason, datetime.now().isoformat(), until_date.isoformat(), message.from_user.id))
...
⏱ مدت: {duration} دقیقه
📝 دلیل: {reason}
👮 سکوت‌کننده: {message.from_user.first_name}
━━━━━━━━━━━━━━━━━━━━━━
""")
...
""")
    ...
    def unmute_command(self, message):
        ...
        try:
            ...
            db.execute("DELETE FROM muted_users WHERE group_id = ? AND user_id = ?", (chat_id, user_id))
            db.execute("UPDATE users SET is_muted = 0, mute_until = '' WHERE group_id = ? AND user_id = ?", (chat_id, user_id))
            db.conn.commit()
            
            bot.reply_to(message, f"""
✅ <b>سکوت کاربر رفع شد!</b>
━━━━━━━━━━━━━━━━━━━━━━
👤 کاربر: {user_name}
🆔 آیدی: <code>{user_id}</code>
━━━━━━━━━━━━━━━━━━━━━━
""")
    ...
    def warn_command(self, message):
        ...
        action = db.get_setting(chat_id, 'warn_action') or 'ban'
...
👤 کاربر: {user_name}
🆔 آیدی: <code>{user_id}</code>
📊 تعداد اخطار: <b>{warns}/{max_warns}</b>
📝 دلیل: {reason}
👮 اخطاردهنده: {message.from_user.first_name}
━━━━━━━━━━━━━━━━━━━━━━
"""
        ...
        if warns >= max_warns:
            if action == 'ban':
                ...
                text += "\n🚨 <b>کاربر به دلیل {max_warns} اخطار بن شد!</b>"
            elif action == 'mute':
                duration = 60
                permissions = telebot.types.ChatPermissions(can_send_messages=False)
                until_date = datetime.now() + timedelta(minutes=duration)
                bot.restrict_chat_member(chat_id, user_id, permissions, until_date=until_date)
                text += f"\n🚨 <b>کاربر به مدت {duration} دقیقه سکوت شد!</b>"
...
🆔 آیدی: <code>{user_id}</code>
📝 دلیل: {reason}
👮 اخراج‌کننده: {message.from_user.first_name}
━━━━━━━━━━━━━━━━━━━━━━
""")
        except Exception as e:
            bot.reply_to(message, f"❌ خطا: {str(e)}")
...
""")
    ...
    def stats_command(self, message):
        ...
        muted = db.fetchall("SELECT * FROM muted_users WHERE group_id = ? AND muted_until > ?", 
                           (chat_id, datetime.now().isoformat()))
        
        total = len(users)
        active = len([u for u in users if u[9] == 0])  # is_banned = 0
        
        text = f"""
📊 <b>آمار گروه</b>
━━━━━━━━━━━━━━━━━━━━━━
📋 <b>اطلاعات:</b>
• نام: {message.chat.title}
• آیدی: <code>{chat_id}</code>
• کاربران کل: {total}
• کاربران فعال: {active}
• بن‌شده: {len(banned)}
...
"""
    ...
    def users_command(self, message):
        chat_id = message.chat.id
        users = db.fetchall("SELECT user_id, first_name, username, messages FROM users WHERE group_id = ? LIMIT 50", (chat_id,))
        ...
        for user in users:
            text += f"• {user[1]} (@{user[2] or 'ندارد'}) - {user[3]} پیام\n"
    ...
    def logs_command(self, message):
        ...
        chat_id = message.chat.id
        logs = db.fetchall("SELECT action, details, created_at FROM logs WHERE group_id = ? ORDER BY id DESC LIMIT 20", (chat_id,))
        
        if not logs:
            bot.reply_to(message, "📭 هیچ لاگی یافت نشد!")
            return
        
        text = "📋 <b>لاگ‌های گروه</b>\n━━━━━━━━━━━━━━━━━━━━━━\n"
        for log in logs:
            text += f"• {log[0]} | {log[1]}\n  📅 {log[2]}\n\n"
    ...
    def rules_command(self, message):
        chat_id = message.chat.id
        result = db.fetchone("SELECT rules FROM groups WHERE id = ?", (chat_id,))
        rules = result[0] if result else "هنوز قوانینی تنظیم نشده است!"
        
        bot.reply_to(message, f"""
📜 <b>قوانین گروه {message.chat.title}</b>
━━━━━━━━━━━━━━━━━━━━━━
{rules}
━━━━━━━━━━━━━━━━━━━━━━
""")
    ...
    def handle_message(self, message):
        ...
        if anti_spam:
            ...
            if len(self.spam_check[key]) > limit:
                ...
                bot.reply_to(message, f"⚠️ <b>اسپم تشخیص داده شد!</b>\nاخطار: {warns[0]}/{max_warns}")
                
                if warns[0] >= max_warns:
                    bot.ban_chat_member(chat_id, user_id)
                    bot.reply_to(message, "🚨 کاربر به دلیل اسپم مکرر بن شد!")
                return
            ...
            if url_pattern.search(text):
                ...
                if allowed:
                    ...
                    if not is_allowed:
                        ...
                        bot.reply_to(message, f"🔗 <b>لینک ممنوع تشخیص داده شد!</b>\nاخطار: {warns[0]}/{max_warns}")
                        
                        if warns[0] >= max_warns:
                            bot.ban_chat_member(chat_id, user_id)
                            bot.reply_to(message, "🚨 کاربر به دلیل ارسال لینک ممنوع بن شد!")
                        return
            ...
            if blocked_words:
                ...
                for word in blocked_words:
                    if word in text_lower:
                        ...
                        bot.reply_to(message, f"📝 <b>کلمه ممنوع تشخیص داده شد!</b>\nاخطار: {warns[0]}/{max_warns}")
                        
                        if warns[0] >= max_warns:
                            bot.ban_chat_member(chat_id, user_id)
                            bot.reply_to(message, "🚨 کاربر به دلیل استفاده از کلمات ممنوع بن شد!")
                        break
...
if __name__ == "__main__":
    print("=" * 60)
    print("🛡️ Luffy Shield Bot - نسخه 7.0.0")
    print("=" * 60)
